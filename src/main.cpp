...
{
    // Woodcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 1000000); // Woodcoin: 1m blocks in ~? years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 2 * 24 * 60 * 60; // Woodcoin: 2 days
static const int64 nTargetSpacing = 2.5 * 60; // Woodcoin: 2.5 minutes
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Woodcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Woodcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // change this to true to generate Genesis Block
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Woodcoin: increase each by adding 2 to bitcoin's value.
...
void static WoodcoinMiner(CWallet *pwallet)
{
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&WoodcoinMiner, pwallet));
}
